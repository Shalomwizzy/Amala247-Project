            ...
            <table class="table">
                <tbody>
                    @foreach ($orderItems as $item)
                        <tr>
                            <td colspan="2"><strong>Order ID:</strong> {{ $item->order_id }}</td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>Customer Name:</strong> {{ $item->user->first_name }} {{ $item->user->last_name }}</td>

                        </tr>
                        <tr>
                            <td colspan="2"><strong>Product Name:</strong> {{ $item->product->product_name }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td><strong>Amount:</strong> &#x20A6;{{ $item->amount }}</td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>Product </strong><img src="{{ asset($item->product->product_image) }}" alt="{{ $item->product->product_name }}" width="100"></td>
                        </tr>
...
<body>
    <div class="container">
        ...
        <div class="order-details">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <!-- User Details -->
                    <tr>
                        <td colspan="3"><strong>Customer Name:</strong> {{ $orderItems->first()->user->first_name }} {{ $orderItems->first()->user->last_name }}</td>
                    </tr>
                    <tr>
                        <td colspan="3"><strong>Phone Number:</strong> {{ $orderItems->first()->user->phone_number }}</td>
                    </tr>
                    <tr>
                        <td colspan="3"><strong>Email:</strong> {{ $orderItems->first()->user->email }}</td>
                    </tr>
                    <tr>
                        <td colspan="3"><strong>Shipping Address:</strong>
                            {{ optional($orderItems->first()->user->userAddress)->street_address ?? '' }},
                            {{ optional($orderItems->first()->user->userAddress)->house_number ?? '' }},
                            {{ optional($orderItems->first()->user->userAddress)->city ?? '' }},
                            {{ optional($orderItems->first()->user->userAddress)->state ?? '' }}
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="copyright">
            <p>&copy; All Rights Reserved at Amala 247</p>
        </div>
    </div>
</body>
</html>
